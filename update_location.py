...
DELAY_NO_LOCATION_MIN = 5
DELAY_FOUND_LOCATION_MIN = 1
...
def determine_nearest_airport(app, latitude, longitude, altitude_m):
    ...
    airports = app.database.airports.find(
        {
...
def determine_status(ground_speed_mps):
    ...
    return "flying" if ground_speed_mps > 25 else "ground"
...
def update_location(app, aircraft_id):
    ...
    def schedule_trip_update_now(app, aircraft_id):
    ...
    last_location = app.database.locations.find_one(
    ...
    if len(data["ac"]) == 0:
        ...
        schedule_trip_update_now(app, aircraft_id)
        return
    ...
    try:
        ...
        location_timestamp = datetime.fromtimestamp(data["now"] / 1000 - ac["seen_pos"])

        # if the location timestamp is within 1 second, skip it
        if last_location_timestamp and abs(
            location_timestamp - last_location_timestamp
        ) < timedelta(seconds=1):
        ...
        app.database.locations.insert_one(location.model_dump())
